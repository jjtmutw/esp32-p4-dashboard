...
/* The Waveshare ESP32-P4 Wi-Fi6 LCD board can power-cycle the host when the
 * hosted reset line is pulsed on every boot. Keep the UI alive and only reset
 * the slave if SDIO initialization really fails.
 */
...
#if !defined(CONFIG_ESP_HOSTED_SDIO_HOST_INTERFACE) && H_SLAVE_RESET_ONLY_IF_NECESSARY
