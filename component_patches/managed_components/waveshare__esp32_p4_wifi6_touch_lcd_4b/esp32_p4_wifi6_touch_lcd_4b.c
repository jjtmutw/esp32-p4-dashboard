...
static lv_indev_t *bsp_display_indev_init(lv_display_t *disp)
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        ESP_LOGW(TAG, "Touch init failed (%s), retry %d/3", esp_err_to_name(ret), i + 1);
        vTaskDelay(pdMS_TO_TICKS(120));
    }

    if (ret != ESP_OK || tp == NULL) {
        ESP_LOGW(TAG, "Touch controller unavailable; starting display without touch input");
        return NULL;
    }
...
lv_display_t *bsp_display_start_with_config(const bsp_display_cfg_t *cfg)
{
    ...
    if (disp_indev == NULL) {
        ESP_LOGW(TAG, "Display started without LVGL touch input device");
    }
